...
def get_gt_images(path: list[str]):
    imgs:list[MatLike] = []
    for i in path:
        name = i.split("/")[-1]
...
def train_generator(train_path: list[str], batch_size: int):
    L = len(train_path)
    while True:
        batch_start = 0
        ...
        while batch_start < L:
            ...
            if X_load is not None and Y_load is not None:
...
def val_generator(val_path: list[str], batch_size: int):
    L = len(val_path)
    while True:
        batch_start = 0
        batch_end = batch_size
...
def dseu():
    ...
    # Bottleneck
    dilate1 = Conv2D(
        256, (3, 3), padding="same", dilation_rate=1, kernel_initializer="he_normal"
    )(pool4)
    dilate1 = BatchNormalization()(dilate1)
    dilate1 = Activation("relu")(dilate1)
    dilate2 = Conv2D(
        256, (3, 3), padding="same", dilation_rate=2, kernel_initializer="he_normal"
    )(pool4)
    dilate2 = BatchNormalization()(dilate2)
    dilate2 = Activation("relu")(dilate2)
    dilate3 = Conv2D(
        256, (3, 3), padding="same", dilation_rate=4, kernel_initializer="he_normal"
    )(pool4)
    dilate3 = BatchNormalization()(dilate3)
    dilate3 = Activation("relu")(dilate3)
    dilate4 = Conv2D(
        256, (3, 3), padding="same", dilation_rate=8, kernel_initializer="he_normal"
    )(pool4)
    ...
    # Decoder
    up5 = Conv2D(512, (2, 2), padding="same", kernel_initializer="he_normal")(
        UpSampling2D(size=(2, 2))(concat_dilate)
    )
    ...
    up6 = Conv2D(256, (2, 2), padding="same", kernel_initializer="he_normal")(
    ...
    up7 = Conv2D(128, (2, 2), padding="same", kernel_initializer="he_normal")(
    ...
    up8 = Conv2D(64, (2, 2), padding="same", kernel_initializer="he_normal")(
...
checkpoint = ModelCheckpoint(savepath, monitor="loss", save_best_only=False)
